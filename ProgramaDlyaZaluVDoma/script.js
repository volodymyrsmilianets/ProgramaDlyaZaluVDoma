...
// === ОНОВЛЕНА ЛОГІКА ВХОДУ З ПЕРЕВІРКОЮ ПАРОЛЯ ===
function loginUser() {
    const username = document.getElementById('loginUsername').value.toLowerCase().trim();
    const password = document.getElementById('loginPassword').value;

    if (!username || !password) {
        alert("Будь ласка, введіть і логін, і пароль.");
        return;
    }

    let users = JSON.parse(localStorage.getItem('TRpointUserCredentials')) || {};
    ...
    if (storedPassword && storedPassword === password) {
        ...
        alert(`Вхід успішний! Ласкаво просимо, ${username}.`);
        checkAuthAndRedirect();
    } else {
...
// === ОНОВЛЕНА ЛОГІКА РЕЄСТРАЦІЇ ЗІ ЗБЕРЕЖЕННЯМ ПАРОЛЯ ===
function registerUser() {
    const username = document.getElementById('regUsername').value.toLowerCase().trim();
    const password = document.getElementById('regPassword').value;

    if (!username || !password || password.length < 6) {
        alert("Будь ласка, введіть логін та пароль (мінімум 6 символів).");
        return;
    }
    ...
    if (users[username]) {
        alert("Користувач з таким логіном вже існує! Спробуйте увійти.");
        return;
    }

    users[username] = password;
    localStorage.setItem('TRpointUserCredentials', JSON.stringify(users));
