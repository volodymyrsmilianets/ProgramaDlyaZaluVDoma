...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Прив'язка нової інтелектуальної функції
...
async function sendMessage() {
    ...
    // Збір даних для передачі в модуль ШІ
    const users = JSON.parse(localStorage.getItem('users')) || {};
    const user = users[activeUser];
    const profile = user.profile || {};

    const userData = {
        name: profile.name || 'Атлет',
        bmi: profile.weight && profile.height ? (profile.weight / ((profile.height / 100) ** 2)).toFixed(1) : "невідомо",
    ...
    messages.innerHTML += `<div class="msg bot-msg" id="${loadingId}">...</div>`;
    messages.scrollTop = messages.scrollHeight;

    // ВИКЛИК ЗОВНІШНЬОГО МОДУЛЯ ШІ
    const botResponse = await getGeminiResponse(userText, userData, Object.keys(exerciseGifs));

    // Обробка команди виклику вправи (якщо вона є у відповіді)
    let finalMessage = botResponse;
    if (botResponse.includes("[EXECUTE_EXERCISE:")) {
        const match = botResponse.match(/\[EXECUTE_EXERCISE:(.+?)\]/);
        if (match) {
            const exName = match[1].trim();
            finalMessage = botResponse.replace(match[0], "");
            if (exerciseGifs[exName]) {
                setTimeout(() => openModal(exerciseGifs[exName], exName), 600);
            }
        }
    }

    document.getElementById(loadingId).innerHTML = finalMessage;
    messages.scrollTop = messages.scrollHeight;
}
